...
def get_captions_by_time(data, start_time=0, end_time=None):
    captions = []
    found_start = False
    ...
    for i, caption in enumerate(data):
        ...
        if not found_start and caption_start <= start_time <= caption_end or caption_end >= end_time:
            if re.search(r'([A-Z]{2,}):\s*(.*)', caption_text):
                found_start = True
            else:
                new_i = i
                while new_i > 0 and not found_start:
                    new_i -= 1
                    new_caption_text = data[new_i]['text']
                    captions.insert(0, new_caption_text)
                    if re.search(r'([A-Z]{2,}):\s*(.*)', new_caption_text):
                        found_start = True
                        break
                found_start = True
        elif not found_start and start_time <= caption_start:
            # If start_time is smaller than the start of the first caption,
            # append captions preceding the specified start_time
            new_i = i
            while new_i > 0 and caption_start > start_time:
                new_i -= 1
                caption_text = data[new_i]['text']
                captions.insert(0, caption_text)
                caption_start = data[new_i]['start']

        if found_start:
            captions.append(caption_text)
            if end_time and caption_end > end_time:
                break

    # Check if the last caption ends with a letter
    last_caption = captions[-1] if captions else ''

    if last_caption and re.search(r'[\w,]\s*$', last_caption):
        i = i + 1
        while i < len(data):
            next_caption = data[i]['text']
            if re.search(r'(([A-Z]{2,}):\s*(.*)|[\.\!\?]\s*$)', next_caption):
...
def combine_captions_into_string(caption_list):
    combined_captions = ' '.join(caption_list)

    # Replace line breaks with spaces unless followed by a parenthesis or speaker name
    combined_captions = re.sub(r'\n(?!([A-Z]{2,}):\s*|\()', ' ', combined_captions)

    # Insert line breaks before speaker names
    combined_captions = re.sub(r'\n?([A-Z]{2,}):\s*', r'\n\1: ', combined_captions)

    # Insert line breaks before or after parentheticals
    combined_captions = re.sub(r'\)\s*', r')\n', combined_captions)
    combined_captions = re.sub(r'\n\(', r'\n\n(', combined_captions)
...
class YoutubeTranscript:
    ...
    def _convert_timestamp_to_seconds(timestamp):
        if isinstance(timestamp, int):
            return timestamp
        if re.match(r'^(\d+h)?\d+m\d+s$', str(timestamp)):
            parts = list(map(int, filter(None, re.split(r'[hms]', str(timestamp)))))
            total_seconds = 0
            for i, part in enumerate(reversed(parts)):
                total_seconds += int(part) * (60 ** i)
        elif re.match(r'^\d+:\d+(:\d+)?$', str(timestamp)):
    ...
    @staticmethod
    def _get_captions_by_time(captions_list, start=0, end=None):
        captions = get_captions_by_time(captions_list, start, end)
        formatted_captions = combine_captions_into_string(captions)
        return formatted_captions

    def get_captions_by_time(self, start='00:00:00', end=None, lang=DEFAULT_LANGUAGE):
        start_seconds = self._convert_timestamp_to_seconds(start)
        end_seconds = self._convert_timestamp_to_seconds(end) if end is not None else end

        captions_list = self.captions_dict.get(lang, [])
        return self._get_captions_by_time(captions_list, start_seconds, end_seconds)
